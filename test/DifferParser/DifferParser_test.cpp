...
TEST(Class_DifferParser, differ_files_can_be_parsed_correctly) {
  ...
  EXPECT_EQ(escapeDifferString, "{'/tmp/differ/oracle.sql':['-- The database is charged and subsequent use requires attention.',' ','-- This sql is used to create a database.','-- database not be installed.',]'/tmp/differ/postgresql.sql':['-- The database is open source and can be used for free and safely.',' ','-- This sql is used to create a database.','DROP TABLE IF EXISTS \"users\";','CREATE TABLE \"public\".\"users\" (','    \"id\" bigint NOT NULL,','   \"name\" text,','   \"age\" integer,','   \"gender\" integer,','   \"city\" text,','   \"country\" text,','   CONSTRAINT \"users_pkey\" PRIMARY KEY (\"id\")',') WITH (oids = false);',]'/tmp/differ/mysql.sql':['-- The database is open source and can be used for free and safely.',' ','-- This sql is used to create a database.','DROP TABLE IF EXISTS `users`;','CREATE TABLE `users` (',' `id` bigint NOT NULL,',' `name` text,',' `age` int DEFAULT NULL,',' `gender` int DEFAULT NULL,',' `city` text,',' `country` text,',' PRIMARY KEY (`id`)',') ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;',]}");
}
